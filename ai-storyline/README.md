...
| Web MVP：对话流UI/选择/数值面板/复盘/剧本选择 | ✅ http://127.0.0.1:8000（Mock模式） |
...
| SSE/NDJSON流式输出 | ✅ 正文增量实时推送（JSON元数据不外泄），失败自动回退非流式重写 |
...
2. **真实玩家体验数据**：把 http://127.0.0.1:8000 发给朋友试玩，/api/admin/metrics 自动对照调研基线
3. 评审通过后打磨：SSE流式输出（当前每场约7s非流式）、路线分享、更多剧本
